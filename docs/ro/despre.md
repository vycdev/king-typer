...
- `koa`: Koa este un web framework pentru node js similar cu express, dar mult mai optimizat și mai concentrat pe middleware. Îl folosesc pentru crearea unui server SPA care să găzduiască SPA-ul pe heroku.
...
- `typescript`: Typescript este un superset de JavaScript strongly-typed. Foarte util în scrierea unui cod fara errori din runtime si erori in general.
...
De asemenea, pentru back end folosim alte instrumente specifice. Chiar daca acestea nu sunt compilate cu scopul de a ii oferi utilizatorului cea mai buna performanta, ele sunt esentiale pentru crearea si mentinerea API-ului.
...
- `knex` - In loc sa interactionam cu baza de date insusi, folosim Knex pe post de query builder. Putem crea migratii, seeduri, si cel mai important, sa interactionam cu baza de date mult mai usor. Knex sanitizeaza inputul si creeaza o metoda usoara, bazata pe functii, de a interactiona cu baza de date.
...
- `joi` - Cand facem requesturi, de multe ori este nevoide de validare. Joi este un middleware creator care efectueaza validarea pentru tine, in acest mod poti sa te asiguri ca primesti datele corecte fara mult efort.
...
Pe partea de front end nu am folosit teste de integrare pentru că nu era necesar deoarece o mulțime de oameni (pe care ii puteti gasi in fisierul README), inclusiv eu, au testat aplicația. De asemenea, cu ajutorul instrumentelor enumerate mai sus, am putut sa evit majoritatea erorilor înainte de a da push pe github.(Am și github actions configurat).
...
# Actualizari de la faza Online 2020 si pana acum.
...
Dar inca este acolo, lafel si toate datele pe care le-am stocat. Doar ca tabul nu mai exista deoarece am considerat ca nu mai este nevoie de el, pagina era invechita, iar designul era facut pentru a arata doar date din localstorage.

## Inbunatatiri pe pagina "Start Typing"

### O noua pagina default pentru pagina de "Start Typing"

Noua pagina de Start Typing are acum ca si default, o pagina care iti arata ultimele scoruri si ultimele recorduri personale. (De asemena poti sa dai click pe numele membrilor din lista pentru a le vizita profilul)
...
Modul easy care contine un test de tastare de 60 de secunde. Are cuvinte aleatorii fara punctuatie sau capitalizare.

Modul normal care contine un test de tastare de 60 de secunde. Are un text luat aleatoriu din baza de date ce poate apartine unei carti, joc, film, etc...
...
Acesta este exact ca si modurile normal si easy doar ca este multiplayer. Despre asta o sa vorbesc mai detaliat un pic mai jos.
...
Cand intri pe unul dintre modurile de multiplayer vei fi pus intr-o lista de asteptare, cand acea lista ajunge a 2 playeri atunci meciul va incepe pentru acei 2 playeri. (Acesta este locul in care am folosit websocketurile)
...
Iar pentru a accesa functia de editare trebuie decat sa dai click pe una din liniile pe care vrei sa le editezi din table. Iar pentru a adauga ceva nou apasa butonul de la sfarsitul listei.

- Panoul 1 este pentru useri, poti sa ii editezi/stergi.
- Panoul 2 este pentru texte, poti sa adaugi/editezi/stergi textele. De asemeanea toate textele sunt acolo, mod easy, normal sau tutorial.
...
# Si acestea sunt noile functionalitati ale aplicatiei King Typer
...
As vrea sa multumesc si tututror testarilor care mi-au oferit feedback in timp ce adaugam functionalitati noi si le editam pe cele vechi.

Mai este cale lunga pana cand as putea sa numesc acest proiect terminat. Mi-ar fi placut sa am mai mult timp pentru a imbunatati frontendul si backendul. A fost destul de greu sa cresc aplicatia din cauza codului meu vechi si in acelasi timp din cauza timpului putin nu am reusit sa fac un design de care sa fiu multumit. O sa continui sa lucrez la proiect chiar si dupa infoeducatie, dar pe moment am vrut sa termin functionalitatea.
